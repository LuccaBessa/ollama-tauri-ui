...
export default function Sidebar({ isOpen }: IProps): JSX.Element {
  ...
  const chatSummaryList: { id: number; title: string; icon: LucideIcon }[] =
    useMemo(() => {
      return data
        ? data!.map((cs) => ({
            id: cs.id,
    ...
    <Collapsible open={isOpen} defaultOpen>
      <CollapsibleContent>
        <div className="flex flex-col w-[300px] h-screen">
          ...
          <Separator />
          <Nav
            chats={chatSummaryList}
            refetch={refetch}
            onClick={(id: number) => getChatInfo(id)}
          />
          <Separator />
          <Dialog>
            ...
            <DialogContent className="min-w-[800px] min-h-[625px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Manage OllamaUI settings and ollama preferences.
                </DialogDescription>
              </DialogHeader>
              <Separator />
              <Settings />
